<?php
// J'ai modifié les \n qui étaient dans les différentes fonctions en <br> car ça ne faisait
...
// Normalement ça a déjà été fait avec le script de base.
...
function printWSBasicInfo($device)
{
    if(isset($device['station_name']))
        echo ("- ".$device['station_name']. " -<br>");
    else if($device['module_name'])
        echo ("- ".$device['module_name']. " -<br>");

    echo ("id: " . $device['_id']. "<br>");
    ...
    if(isset($device['type']))
    {
        ...
        {
            // Outdoor Module
            case "NAModule1": echo ("Outdoor<br>");
                              break;
            //Wind Sensor
            case "NAModule2": echo("Wind Sensor<br>");
                              break;

            //Rain Gauge
            case "NAModule3": echo("Rain Gauge<br>");
                              break;
            //Indoor Module
            case "NAModule4": echo("Indoor<br>");
                              break;
            case "NAMain" : echo ("Main device <br>");
                            break;
        }
...
function printThermBasicInfo($dev)
{
    ...
    foreach($dev['modules'] as $module)
    {
        ...
        foreach($module['therm_program_list'] as $program)
        {
            ...
            echo ("        id: ".$program['program_id']." <br>");
            if(isset($program['selected']) && $program['selected'] === TRUE)
            {
                echo "         This is the current program <br>";
            }
        }
    }
...
function printPersonInformation(Netatmo\Objects\NAPerson $person, $tz)
{
    ...
    if($person->isAway())
        echo("is away from home <br>" );
    else echo("is home <br>");
...
function printCameraInformation(Netatmo\Objects\NACamera $camera)
{
    ...
    echo("id: ". $camera->getId() ."<br>");
    echo("Monitoring status: ". $camera->getVar(Netatmo\Common\NACameraInfo::CI_STATUS) ."<br>");
    echo("SD card status: " .$camera->getVar(Netatmo\Common\NACameraInfo::CI_SD_STATUS) . "<br>");
    echo ("Power status: ". $camera->getVar(Netatmo\Common\NACameraInfo::CI_ALIM_STATUS) ."<br>");

    if($camera->getGlobalStatus())
        $globalStatus = "OK";
    else $globalStatus = "NOK";

    echo ("Global Status: ". $globalStatus ."<br>");
